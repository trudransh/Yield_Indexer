...
export const config = {
  ...
  rpcUrl: assertNotNull(
    process.env.RPC_ARBITRUM_HTTP,
    "RPC_ARBITRUM_HTTP env var is required"
  ),
  ...
  stablecoins: new Map<string, string>([
...
export function poolId(chainId: number, address: string): string {
  return `${chainId}:${address.toLowerCase()}`;
}

export function snapshotId(chainId: number, address: string, blockNumber: number): string {
